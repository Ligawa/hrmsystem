...
// This is a protected admin endpoint to set up the database
...
export async function POST(request: NextRequest) {
  try {
    // Verify the request has the correct admin token
    const authHeader = request.headers.get('authorization')
    const adminToken = process.env.SETUP_ADMIN_TOKEN
    ...
    console.log('[v0] Executing database setup...')
  ...
  return NextResponse.json(
    {
      ...
      instructions: 'POST to this endpoint with Authorization header: Bearer {SETUP_ADMIN_TOKEN}',
    },
    { status: 200 },
  )
}
